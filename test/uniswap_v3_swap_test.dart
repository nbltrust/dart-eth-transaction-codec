...
Future<ParsedResult> parseExactInput(ContractCall callInfo, dynamic tx) async {
  ...
  final args = [
    [(amountIn / Decimal.parse('1e' + tokenIn.params?['decimal'])).toDecimal().toString(), symbolIn],
    ['≥', (amountOutMinimum / Decimal.parse('1e' + tokenOut.params?['decimal'])).toDecimal().toString(), tokenOut.symbol],
  ];
...
Future<ParsedResult> parseExactOutput(ContractCall callInfo, dynamic tx) async {
  ...
  final args = [
    ['≤', (amountInMaximum / Decimal.parse('1e' + tokenIn.params?['decimal'])).toDecimal().toString(), symbolIn],
    [(amountOut / Decimal.parse('1e' + tokenOut.params?['decimal'])).toDecimal().toString(), tokenOut.symbol],
  ];
