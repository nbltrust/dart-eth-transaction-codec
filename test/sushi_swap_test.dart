...
Future<ParsedResult> parseSwap(ContractCall callInfo, dynamic tx) async {
  ...
  final amountInKey = params.keys.firstWhere((k) => k.startsWith('amountIn'), orElse: () => null);
  ...
  final args = [
    [amountOutKey.endsWith('Min') ? '' : '≤', amountIn / Decimal.parse('1e' + tokenIn.params['decimal']), symbolIn],
    [amountOutKey.endsWith('Min') ? '≥' : '', amountOut / Decimal.parse('1e' + tokenOut.params['decimal']), symbolOut],
  ];
